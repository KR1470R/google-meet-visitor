...
const nonWindowsMultipleCalls = async (options: any = {}) => {
  ...
  await Promise.all(
    ["comm", "args", "ppid", "uid", "%cpu", "%mem"].map(async (cmd: string) => {
      ...
      for (let line of stdout.trim().split("\n").slice(1)) {
        line = line.trim();
        const pid = line.split(" ", 1)[0] as keyof typeof returnValue;
        ...
        (returnValue[pid] as any)[cmd] = value;
      }
    })
  );
...
const nonWindowsCall = async (options: any = {}) => {
  ...
  for (const line of psArgsLines) {
    ...
    processCmds[pid] = (cmds as unknown as string[]).join(" ");
  }
  ...
  const processes = psLines
    .map((line) => {
      ...
      const { pid, ppid, uid, cpu, memory, comm } = match.groups as {
        pid: string;
        ppid: string;
        uid: string;
        cpu: string;
